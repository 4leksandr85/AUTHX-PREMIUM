...
-- //  Offsets: version-ddf602d9cfe44005 | 11/08/2026
-- ████████████████████████████████████████████████████
...
local tabNames = {"AIMBOT","TRIGGERBOT","ESP","MOVEMENT","MISC","SKIN"}
local tabPages = {}
local tabBtns = {}
...
mkKeybindRow(c5a,"Toggle Menu","INSERT",2)
...
-- Colour presets that ship as named "skins"
local WEAPON_SKINS = {
    {name="Default",        color=nil,                          mat=Enum.Material.SmoothPlastic, neon=false},
    {name="Obsidian",       color=Color3.fromRGB(12,12,12),     mat=Enum.Material.SmoothPlastic, neon=false},
    {name="Arctic White",   color=Color3.fromRGB(240,240,240),  mat=Enum.Material.SmoothPlastic, neon=false},
    {name="Blood Red",      color=Color3.fromRGB(180,10,10),    mat=Enum.Material.SmoothPlastic, neon=false},
    {name="Cobalt",         color=Color3.fromRGB(30,80,200),    mat=Enum.Material.SmoothPlastic, neon=false},
    {name="Poison",         color=Color3.fromRGB(50,200,50),    mat=Enum.Material.Neon,          neon=true },
    {name="Solar",          color=Color3.fromRGB(255,140,0),    mat=Enum.Material.Neon,          neon=true },
    {name="Void",           color=Color3.fromRGB(80,0,180),     mat=Enum.Material.Neon,          neon=true },
    {name="Ice",            color=Color3.fromRGB(180,230,255),  mat=Enum.Material.Glass,         neon=false},
    {name="Gold",           color=Color3.fromRGB(255,200,40),   mat=Enum.Material.Metal,         neon=false},
    {name="Midnight",       color=Color3.fromRGB(10,10,40),     mat=Enum.Material.Metal,         neon=false},
    {name="Rose",           color=Color3.fromRGB(255,100,150),  mat=Enum.Material.SmoothPlastic, neon=false},
    {name="Crimson Neon",   color=Color3.fromRGB(255,20,60),    mat=Enum.Material.Neon,          neon=true },
    {name="Cyan Neon",      color=Color3.fromRGB(0,255,220),    mat=Enum.Material.Neon,          neon=true },
    {name="Ghost",          color=Color3.fromRGB(200,200,200),  mat=Enum.Material.Glass,         neon=false},
    {name="Stealth",        color=Color3.fromRGB(20,20,20),     mat=Enum.Material.Metal,         neon=false},
    {name="Inferno",        color=Color3.fromRGB(255,60,0),     mat=Enum.Material.Neon,          neon=true },
    {name="Forest",         color=Color3.fromRGB(30,90,30),     mat=Enum.Material.SmoothPlastic, neon=false},
    {name="Chrome",         color=Color3.fromRGB(200,210,220),  mat=Enum.Material.Metal,         neon=false},
    {name="Rainbow",        color=nil,                          mat=Enum.Material.Neon,          neon=true },
}

-- Material options
local MAT_OPTIONS = {
    "SmoothPlastic","Metal","Glass","Neon","Wood","Marble","Granite","Brick","DiamondPlate","Fabric"
}
local MAT_MAP = {}
for _,m in ipairs(MAT_OPTIONS) do MAT_MAP[m]=Enum.Material[m] end

-- Core apply function — walks the player's held tool and overrides every BasePart
local rainbowHue = 0
local function getHeldTool()
    local char = LocalPlayer.Character
    if not char then return nil end
    -- Check character children for a Tool
    for _, v in ipairs(char:GetChildren()) do
        if v:IsA("Tool") then return v end
    end
    -- Also check backpack for preview (won't apply visually but noted)
    return nil
end
...
local function applyWeaponSkin()
    ...
    for _, part in ipairs(tool:GetDescendants()) do
        if part:IsA("BasePart") and part.Name ~= "Handle" or (part:IsA("BasePart") and part.Name == "Handle") then
            if part:IsA("BasePart") then
                if WS.TintEnabled and WS.TintColor then
                    pcall(function() part.Color = WS.TintColor end)
                end
                if WS.MatEnabled then
                    pcall(function() part.Material = WS.Material end)
                end
                if WS.TransEnabled then
                    pcall(function() part.Transparency = WS.Transparency end)
                end
                if WS.ReflEnabled then
                    pcall(function() part.Reflectance = WS.Reflectance end)
                end
            end
        end
    end
...
local function resetWeaponSkin()
    local tool = getHeldTool()
    if not tool then return end
    for _, part in ipairs(tool:GetDescendants()) do
        if part:IsA("BasePart") then
    ...
    WS.ActiveSkin = "Default"
end

local function applySkinPreset(preset)
    if preset.name == "Default" then resetWeaponSkin() return end
    local tool = getHeldTool()
    if not tool then return end
    local isRainbow = preset.name == "Rainbow"
    for _, part in ipairs(tool:GetDescendants()) do
        if part:IsA("BasePart") then
            pcall(function()
                if not isRainbow and preset.color then part.Color = preset.color end
                part.Material     = preset.mat
                part.Transparency = 0
                part.Reflectance  = preset.neon and 0 or 0.3
            end)
        end
    end
    WS.TintEnabled = not isRainbow and preset.color ~= nil
    WS.TintColor   = preset.color or WS.TintColor
    WS.MatEnabled  = true
    WS.Material    = preset.mat
    WS.NeonEnabled = preset.neon
    WS.RainbowMode = isRainbow
    WS.ActiveSkin  = preset.name
end

-- Rainbow loop
RunService.Heartbeat:Connect(function(dt)
    if not WS.RainbowMode then return end
    ...
    for _, part in ipairs(tool:GetDescendants()) do
        if part:IsA("BasePart") then
            pcall(function() part.Color = col part.Material = Enum.Material.Neon end)
        end
    end
end)
...
-- Active skin display card
local c6a,_ = mkCard(p6,1)
lbl(c6a,{Text="WEAPON SKIN",Font=Enum.Font.GothamBold,TextSize=9,TextColor3=PAL.textM,
    Size=UDim2.new(1,0,0,16),LayoutOrder=0})
local activeLbl = lbl(c6a,{
    Text="Active: Default  |  Hold a weapon then hit APPLY",
    Font=Enum.Font.Gotham,TextSize=10,TextColor3=PAL.textM,
    Size=UDim2.new(1,0,0,14),LayoutOrder=1,
})

-- Tint controls
local c6b,_ = mkCard(p6,2)
lbl(c6b,{Text="TINT OVERRIDE",Font=Enum.Font.GothamBold,TextSize=9,TextColor3=PAL.textM,
    Size=UDim2.new(1,0,0,16),LayoutOrder=0})
mkToggleRow(c6b,"Enable Tint","TintEnabled",1)

-- inline RGB for weapon tint reusing mkColorRow approach
local tintWrapper = inst("Frame",{
    Size=UDim2.new(1,0,0,0),AutomaticSize=Enum.AutomaticSize.Y,
    BackgroundTransparency=1,BorderSizePixel=0,LayoutOrder=2,Parent=c6b,
})
local tintHeader = mkRow(tintWrapper,0)
lbl(tintHeader,{Text="Tint Color",Font=Enum.Font.Gotham,TextSize=12,TextColor3=PAL.textS,Size=UDim2.new(0.7,0,1,0)})
local tintPreview = inst("Frame",{
    Size=UDim2.new(0,28,0,14),AnchorPoint=Vector2.new(1,0.5),Position=UDim2.new(1,0,0.5,0),
    BackgroundColor3=WS.TintColor,BorderSizePixel=0,Parent=tintHeader,
}) corner(tintPreview,4) stroke(tintPreview,PAL.border)

for chIdx, chName in ipairs({"R","G","B"}) do
    local initV = chIdx==1 and math.floor(WS.TintColor.R*255) or chIdx==2 and math.floor(WS.TintColor.G*255) or math.floor(WS.TintColor.B*255)
    local sub=inst("Frame",{Size=UDim2.new(1,0,0,28),BackgroundTransparency=1,BorderSizePixel=0,LayoutOrder=chIdx,Parent=tintWrapper})
    lbl(sub,{Text=chName,Font=Enum.Font.GothamBold,TextSize=10,TextColor3=PAL.textM,Size=UDim2.new(0,16,1,0),TextXAlignment=Enum.TextXAlignment.Center})
    local vL2=lbl(sub,{Text=tostring(initV),Font=Enum.Font.Gotham,TextSize=10,TextColor3=PAL.textM,Size=UDim2.new(0,28,1,0),Position=UDim2.new(1,-28,0,0),TextXAlignment=Enum.TextXAlignment.Right})
    local tBg2=inst("Frame",{Size=UDim2.new(1,-48,0,3),Position=UDim2.new(0,20,0.5,0),BackgroundColor3=PAL.bg3,BorderSizePixel=0,Parent=sub}) corner(tBg2,4)
    local pct2=(initV/255)
    local fl2=inst("Frame",{Size=UDim2.new(pct2,0,1,0),BackgroundColor3=PAL.accent,BorderSizePixel=0,Parent=tBg2}) corner(fl2,4)
    local th2=inst("Frame",{Size=UDim2.new(0,11,0,11),AnchorPoint=Vector2.new(0.5,0.5),Position=UDim2.new(pct2,0,0.5,0),BackgroundColor3=PAL.accent,BorderSizePixel=0,ZIndex=3,Parent=tBg2}) corner(th2,8)
    local curCh=initV local drag4=false
    local function upd4(pos)
        local ratio=math.clamp((pos.X-tBg2.AbsolutePosition.X)/math.max(tBg2.AbsoluteSize.X,1),0,1)
        curCh=math.floor(ratio*255) fl2.Size=UDim2.new(ratio,0,1,0) th2.Position=UDim2.new(ratio,0,0.5,0) vL2.Text=tostring(curCh)
        local c=WS.TintColor
        WS.TintColor=Color3.fromRGB(
            chIdx==1 and curCh or math.floor(c.R*255),
            chIdx==2 and curCh or math.floor(c.G*255),
            chIdx==3 and curCh or math.floor(c.B*255)
        )
        tintPreview.BackgroundColor3=WS.TintColor
    end
    local sb4=inst("TextButton",{Size=UDim2.new(1,0,1,0),BackgroundTransparency=1,Text="",ZIndex=4,Parent=sub})
    sb4.InputBegan:Connect(function(i) if i.UserInputType==Enum.UserInputType.MouseButton1 then drag4=true upd4(i.Position) end end)
    UserInputService.InputEnded:Connect(function(i) if i.UserInputType==Enum.UserInputType.MouseButton1 then drag4=false end end)
    UserInputService.InputChanged:Connect(function(i) if drag4 and i.UserInputType==Enum.UserInputType.MouseMovement then upd4(i.Position) end end)
end

-- Material / misc controls
local c6c,_ = mkCard(p6,3)
lbl(c6c,{Text="MATERIAL & SURFACE",Font=Enum.Font.GothamBold,TextSize=9,TextColor3=PAL.textM,
    Size=UDim2.new(1,0,0,16),LayoutOrder=0})
mkToggleRow(c6c,"Enable Material","MatEnabled",1)
mkDropRow(c6c,"Material",MAT_OPTIONS,"MatEnabled",2,function(v) WS.Material=MAT_MAP[v] or Enum.Material.SmoothPlastic end)
mkToggleRow(c6c,"Rainbow Mode","RainbowMode",3)
mkToggleRow(c6c,"Enable Transparency","TransEnabled",4)
mkSliderRow(c6c,"Transparency","Transparency",0,0.9,function(v) return ("%.1f"):format(v) end,5)
mkToggleRow(c6c,"Enable Reflectance","ReflEnabled",6)
mkSliderRow(c6c,"Reflectance","Reflectance",0,1.0,function(v) return ("%.1f"):format(v) end,7)

-- Apply / Reset buttons
local c6d,_ = mkCard(p6,4)
lbl(c6d,{Text="CONTROLS",Font=Enum.Font.GothamBold,TextSize=9,TextColor3=PAL.textM,
    Size=UDim2.new(1,0,0,16),LayoutOrder=0})
local btnRow2=inst("Frame",{Size=UDim2.new(1,0,0,34),BackgroundTransparency=1,BorderSizePixel=0,LayoutOrder=1,Parent=c6d})
local applyWBtn=inst("TextButton",{
    Size=UDim2.new(0.48,0,0,26),Position=UDim2.new(0,0,0.5,-13),
    BackgroundColor3=PAL.accent,BorderSizePixel=0,
    Text="APPLY TO WEAPON",Font=Enum.Font.GothamBold,TextSize=10,TextColor3=PAL.bg0,
    Parent=btnRow2,
}) corner(applyWBtn,6)
local resetWBtn=inst("TextButton",{
    Size=UDim2.new(0.48,0,0,26),Position=UDim2.new(0.52,0,0.5,-13),
    BackgroundColor3=PAL.bg3,BorderSizePixel=0,
    Text="RESET",Font=Enum.Font.GothamBold,TextSize=11,TextColor3=PAL.textS,
    Parent=btnRow2,
}) corner(resetWBtn,6) stroke(resetWBtn,PAL.border)
applyWBtn.MouseButton1Click:Connect(function()
    applyWeaponSkin()
    activeLbl.Text="Active: "..WS.ActiveSkin.."  |  Applied"
end)
resetWBtn.MouseButton1Click:Connect(function()
    resetWeaponSkin()
    WS.RainbowMode=false
    activeLbl.Text="Active: Default  |  Reset"
end)

-- Skin presets grid
local c6e,_ = mkCard(p6,5)
lbl(c6e,{Text="SKIN PRESETS",Font=Enum.Font.GothamBold,TextSize=9,TextColor3=PAL.textM,
    Size=UDim2.new(1,0,0,16),LayoutOrder=0})

local gridFrame=inst("Frame",{
    Size=UDim2.new(1,0,0,0),AutomaticSize=Enum.AutomaticSize.Y,
    BackgroundTransparency=1,BorderSizePixel=0,LayoutOrder=1,Parent=c6e,
})
local gridLayout=inst("UIGridLayout",{
...
for idx, skin in ipairs(WEAPON_SKINS) do
    local sb=inst("TextButton",{
        Size=UDim2.new(0,0,0,0), -- grid handles size
        BackgroundColor3=skin.color or PAL.bg3,
        BorderSizePixel=0,
        Text=skin.name,
        Font=Enum.Font.GothamBold,
        TextSize=10,
        TextColor3=skin.neon and Color3.fromRGB(0,0,0) or PAL.textP,
        AutoButtonColor=false,
        LayoutOrder=idx,
        Parent=gridFrame,
    })
    corner(sb,6)
    if not skin.color then stroke(sb,PAL.border) end
    sb.MouseEnter:Connect(function()
        TweenService:Create(sb,TweenInfo.new(0.1),{BackgroundColor3=skin.color and skin.color:Lerp(Color3.fromRGB(255,255,255),0.15) or PAL.bg2}):Play()
    end)
    sb.MouseLeave:Connect(function()
        TweenService:Create(sb,TweenInfo.new(0.1),{BackgroundColor3=skin.color or PAL.bg3}):Play()
    end)
    sb.MouseButton1Click:Connect(function()
        applySkinPreset(skin)
        activeLbl.Text="Active: "..skin.name..(skin.neon and "  [NEON]" or "")
    end)
end
...
print("[AuthX v3] Loaded — INSERT=Menu | RSHIFT=Aim | F1=ESP | F2=Trig | F3=Silent | DEL=Panic")
